...
cameras <- import("L:/BBMR_CSV_FILE_20220702.CSV")
...
df <- cameras %>%
  ...
  mutate(Status = case_when(Status == "P" ~ "Paid in full", 
                            Status == "O" ~ "Open",
                            (Status == "A" & is.na(`Paid Date`)) ~ "Balance abated",
                             TRUE ~ Status))
...
export_excel(issued, "Cameras", "outputs/Camera Violations 2022-06-02.xlsx", "new")
